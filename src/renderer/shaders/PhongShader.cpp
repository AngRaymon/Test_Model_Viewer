...
const char* PhongShader::vertShaderSrc = 
	"#version 120\n										"\
...
const char* PhongShader::fragShaderSrc = 
	"#version 120\n										"\
	"varying vec3 normalV;\n							"\
	"varying vec2 uvOut;\n								"\
	"uniform sampler2D tex;\n							"\
	"const vec3 lightV = vec3(-1.0, -0.3, 0.5);\n		"\
	"void main(){\n										"\
...
void PhongShader::Activate() {
	glUseProgram(m_program);
}

void PhongShader::Deactivate() {
	glUseProgram(0);
}
